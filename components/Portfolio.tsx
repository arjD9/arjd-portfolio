          ...
          {EXPERIENCES.map((exp,i)=>(
            <motion.div key={exp.id} initial={{opacity:0}} animate={{opacity:1}} transition={{delay:0.3+i*0.08}}>
              <button onClick={()=>setOpenId(openId===exp.id?null:exp.id)} data-hover
                ...
                <div className="py-5 grid grid-cols-[minmax(0,1fr)_120px_20px] items-start gap-6">
                  ...
                  <div className="text-right flex-shrink-0 hidden sm:block">                    <p className={`text-[11px] font-mono mt-0.5 ${dark?'text-white/20':'text-black/20'}`}>{exp.location}</p>
                  </div>
